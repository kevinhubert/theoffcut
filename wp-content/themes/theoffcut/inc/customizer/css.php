...
class ATIK_CSS {
	...
	public function add( $data ) {
		 ...
		 * @param array    $data    The selectors and properties to add to the CSS.
		 */
		$data  = apply_filters( 'atik_css_add', $data );
	...
	private function parse_selectors( $selectors, $tab = '' ) {
		 ...
		 * thus be trusted to be sanitary.
		 */
		$n      = $this->line_ending;
	...
	private function parse_declarations( $declarations, $tab = '' ) {
		 ...
		 * routines are implemented when the developer calls `atik_get_css->add()`. Because every property value has
		...
		foreach ( $declarations as $property => $value ) {
			 ...
			 */
			$output .= apply_filters( 'atik_css_parse_declaration', $parsed_value, $property, $value, $t, $n );
		}
		 ...
		 */
		return apply_filters( 'atik_css_parse_declarations', $output, $declarations, $tab );
	}
}
